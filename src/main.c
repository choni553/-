 ...
 * @param argv The arguments
 */
static void kill_system_demo(void)
{
    /* Kill rk356x-demo if running, to free the framebuffer */
    FILE *fp = popen("pidof rk356x-demo 2>/dev/null", "r");
    if (fp) {
        char pid[16] = {0};
        if (fgets(pid, sizeof(pid), fp) && pid[0]) {
            int p = atoi(pid);
            if (p > 0) kill(p, SIGKILL);
        }
        pclose(fp);
    }
    /* Also kill the launch script */
    system("pkill -9 -f S50launch_demo 2>/dev/null");
}
