...
static int is_game_over(void){for(int r=0;r<SZ;r++)for(int c=0;c<SZ;c++){if(!grid[r][c])return 0;if(c+1<SZ&&grid[r][c]==grid[r][c+1])return 0;if(r+1<SZ&&grid[r][c]==grid[r+1][c])return 0;}return 1;}
static void move(int dr,int dc){int mv=0;
    ...
    if(mv){add();buzz(15);draw();int won=0;for(int i=0;i<SZ;i++)for(int j=0;j<SZ;j++)if(grid[i][j]>=2048)won=1;if(won){lv_label_set_text_fmt(g_score,"You Win! %d Hi:%d",score,hi);}else{lv_label_set_text_fmt(g_score,"Score:%d Hi:%d",score,hi);if(score>hi)hi=score;}}else if(is_game_over()){lv_label_set_text_fmt(g_score,"Game Over! %d Hi:%d",score,hi);buzz(200);}}
...
void ui_2048_screen_init(void){
    ...
    lv_obj_add_event_cb(ui_2048,gest,LV_EVENT_GESTURE,NULL);lv_obj_add_event_cb(g_canvas,gest,LV_EVENT_GESTURE,NULL);reset();}
